...
namespace Huli::Vulkan
{
        ...
        : device_{device}
    {
        ...
        const uint32_t width =
            !attachments.empty() ? attachments[0]->vkExtents().width : depthAttachment->vkExtents().width;
        const uint32_t height =
            !attachments.empty() ? attachments[0]->vkExtents().height : depthAttachment->vkExtents().height;
        ...
        context.setVkObjectname(framebuffer_, VK_OBJECT_TYPE_FRAMEBUFFER, "Framebuffer: " + name);
    }
