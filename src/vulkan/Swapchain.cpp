...
namespace Huli::Vulkan
{
    ...
    Swapchain::Swapchain(const Context& context,
        ...
        : device_{context.device()},
          ...
          extent_{extent}
    {
        const uint32_t numImages = std::clamp(physicalDevice.surfaceCapabilities().minImageCount + 1,
    ...
    std::shared_ptr<Texture> Swapchain::acquireImage()
    {
        ...
        VK_CHECK(vkAcquireNextImageKHR(device_, swapchain_, UINT64_MAX, imageAvailable_, acquireFence_, &imageIndex_));
    ...
    void Swapchain::createTextures(const Context& context, VkFormat imageFormat, const VkExtent2D& extent)
    {
        ...
        {
            images_.emplace_back(std::make_shared<Texture>(context,
                                                           device_,
                                                           images[index],
                                                           imageFormat,
                                                           VkExtent3D{
    ...
    void Swapchain::createSemaphores(const Context& context)
    {
        ...
        context.setVkObjectname(
            imageAvailable_, VK_OBJECT_TYPE_SEMAPHORE, "Semaphore: swapchain image available semaphore");

        VK_CHECK(vkCreateSemaphore(device_, &semaphoreInfo, nullptr, &imageRendered_));
        context.setVkObjectname(
            imageRendered_, VK_OBJECT_TYPE_SEMAPHORE, "Semaphore: swapchain image presented semaphore");
    }
