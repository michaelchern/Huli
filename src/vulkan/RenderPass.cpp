...
namespace Huli::Vulkan
{
        ...
        : device_{context.device()}
    {
        ...
        for (uint32_t index = 0; index < attachments.size(); ++index)
        {
            attachmentDescriptors.emplace_back(VkAttachmentDescription{
                ...
                .stencilLoadOp = attachments[index]->isStencil() ? loadOp[index] : VK_ATTACHMENT_LOAD_OP_DONT_CARE,
                .stencilStoreOp = attachments[index]->isStencil() ? storeOp[index] : VK_ATTACHMENT_STORE_OP_DONT_CARE,
        ...
        const VkSubpassDescription spd = {
            ...
            .pDepthStencilAttachment =
                depthStencilAttachmentReference.has_value() ? &depthStencilAttachmentReference.value() : nullptr,
        };
        ...
        const VkRenderPassCreateInfo rpci = {
            ...
            .pSubpasses = &spd,
            .dependencyCount = 2,
            .pDependencies = dependencies.data(),  // being extra liberal with this, in production this
    ...
    RenderPass::RenderPass(const Context& context,
                           ...
                           uint32_t stencilAttachmentIndex,
        ...
        : device_{context.device()}
    {
        const bool sameSizes = formats.size() == initialLayouts.size() && formats.size() == finalLayouts.size() &&
        ...
        for (uint32_t index = 0; index < formats.size(); ++index)
        {
            attachmentDescriptors.emplace_back(VkAttachmentDescription{
                ...
                .loadOp = loadOp[index],
                .storeOp = storeOp[index],
                .stencilLoadOp = index == stencilAttachmentIndex ? stencilLoadOp : VK_ATTACHMENT_LOAD_OP_DONT_CARE,
                .stencilStoreOp = index == stencilAttachmentIndex ? stencilStoreOp : VK_ATTACHMENT_STORE_OP_DONT_CARE,
        ...
        : device_{context.device()}
    {
        ...
        VK_CHECK(vkCreateRenderPass(device_, &rpci, nullptr, &renderPass_));
        context.setVkObjectname(renderPass_, VK_OBJECT_TYPE_RENDER_PASS, "Render pass (fdm support): " + name);
    }
