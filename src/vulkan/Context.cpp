...
namespace Huli::Vulkan
{
    ...
    VkPhysicalDeviceFragmentDensityMapOffsetFeaturesQCOM Context::fragmentDensityMapOffsetFeatures_ = {
        ...
        : printEnumerations_{printEnumerations}
    {
        ...
        std::transform(
            enabledLayers_.begin(), enabledLayers_.end(), instanceLayers.begin(), std::mem_fn(&std::string::c_str));

        {
            ...
            std::transform(enabledInstanceExtensions_.begin(),
            ...
            const std::array<const char*, 1> setting_gpu_based_action = {"GPU_BASED_DEBUG_PRINTF"};
...
// Surface
        ...
        physicalDevice_ = choosePhysicalDevice(enumeratePhysicalDevices(requestedDeviceExtensions, enableRayTracing),
        ...
        if (physicalDevice_.graphicsFamilyIndex().has_value())
        {
            if (physicalDevice_.graphicsFamilyCount() > 0)
            {
                ...
                {
                    vkGetDeviceQueue(
                        device_, physicalDevice_.graphicsFamilyIndex().value(), uint32_t(i), &graphicsQueues_[i]);
                }
            }
        }
        if (physicalDevice_.computeFamilyIndex().has_value())
        {
            if (physicalDevice_.computeFamilyCount() > 0)
            {
                ...
                {
                    vkGetDeviceQueue(
                        device_, physicalDevice_.computeFamilyIndex().value(), uint32_t(i), &computeQueues_[i]);
                }
            }
        }
        if (physicalDevice_.transferFamilyIndex().has_value())
        {
            if (physicalDevice_.transferFamilyCount() > 0)
            {
                ...
                {
                    vkGetDeviceQueue(
                        device_, physicalDevice_.transferFamilyIndex().value(), uint32_t(i), &transferQueues_[i]);
                }
            }
        }
        if (physicalDevice_.sparseFamilyIndex().has_value())
        {
            if (physicalDevice_.sparseFamilyCount() > 0)
            {
                ...
                {
                    vkGetDeviceQueue(
                        device_, physicalDevice_.sparseFamilyIndex().value(), uint32_t(i), &sparseQueues_[i]);
                }
            }
        }
        if (physicalDevice_.presentationFamilyIndex().has_value())
        {
            vkGetDeviceQueue(device_, physicalDevice_.presentationFamilyIndex().value(), 0, &presentationQueue_);
        }
    ...
    {
        physicalDevice_ =
            PhysicalDevice(vkPhysicalDevice, VK_NULL_HANDLE, requestedDeviceExtensions, printEnumerations_, false);
        ...
        // Create the device
        {
            ...
            bufferDeviceAddrFeature.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES;
    ...
    void Context::createSwapchain(VkFormat format,
                                  ...
                                  const VkExtent2D& extent)
    {
        ...
        swapchain_ = std::make_unique<Swapchain>(
            *this, physicalDevice_, surface_, presentationQueue_, format, colorSpace, presentMode, extent);
    }
    ...
    std::shared_ptr<Buffer> Context::createPersistentBuffer(size_t size,
                                                            ...
                                                            const std::string& name) const
    {
        const VkMemoryPropertyFlags cpuVisibleMemoryFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                                         ...
                                                         const std::string& name) const
    {
        ...
        return std::make_shared<Buffer>(
            this, memoryAllocator(), stagingBufferCreateInfo, stagingAllocationCreateInfo, name);
    }
    ...
    std::shared_ptr<Buffer> Context::createStagingBuffer(VkDeviceSize size,
                                                         ...
                                                         const std::string& name) const
    {
        return std::make_shared<Buffer>(this, memoryAllocator(), size, usage, actualBuffer, name);
    }
    ...
    void Context::uploadToGPUBuffer(CommandQueueManager& queueMgr,
                                    ...
                                    const void* data,
                                    long totalSize,
                                    uint64_t gpuBufferOffset) const
    {
        auto stagingBuffer =
            createStagingBuffer(totalSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, gpuBuffer, "staging buffer");
    ...
    std::shared_ptr<Texture> Context::createTexture(VkImageType type,
                                                    VkFormat format,
    ...
    std::shared_ptr<Sampler> Context::createSampler(VkFilter minFilter,
                                                    VkFilter magFilter,
                                                    ...
                                                    const std::string& name) const
    {
        return std::make_shared<Sampler>(
            *this, minFilter, magFilter, addressModeU, addressModeV, addressModeW, maxLod, name);
    }
                                                            ...
                                                            int graphicsQueueIndex)
    {
        ...
        return CommandQueueManager(*this,
                                   device_,
                                   count,
    ...
    CommandQueueManager Context::createTransferCommandQueue(uint32_t count,
                                                            uint32_t concurrentNumCommands,
                                                            const std::string& name,
    ...
    std::shared_ptr<Pipeline> Context::createGraphicsPipeline(const Pipeline::GraphicsPipelineDescriptor& desc,
    ...
    std::shared_ptr<Pipeline> Context::createComputePipeline(const Pipeline::ComputePipelineDescriptor& desc,
                                                             const std::string& name /*= ""*/)
    {
        return std::make_shared<Pipeline>(this, desc, name);
    }

    std::shared_ptr<Pipeline> Context::createRayTracingPipeline(const Pipeline::RayTracingPipelineDescriptor& desc,
                                                                const std::string& name /*= ""*/)
    {
        return std::make_shared<Pipeline>(this, desc, name);
    }
        ...
        const std::string& name) const
    {
        return std::make_shared<RenderPass>(
            *this, attachments, resolveAttachments, loadOp, storeOp, layout, bindPoint, name);
    }
        ...
        const std::string& name) const
    {
        return std::make_unique<Framebuffer>(
            *this, device_, renderPass, colorAttachments, depthAttachment, stencilAttachment, name);
    }

    void Context::beginDebugUtilsLabel(VkCommandBuffer commandBuffer,
                                       const std::string& name,
    ...
    [[nodiscard]] std::vector<std::string> Context::enumerateInstanceExtensions()
    {
        ...
        std::vector<std::string> returnValues;
        std::transform(extensionProperties.begin(),
        ...
        {
            std::cerr << "Found " << extensionsCount << " extension(s) for the instance" << std::endl;
            for (const auto& layer : returnValues)
            {
                std::cerr << "\t" << layer << std::endl;
            }
    ...
    std::vector<PhysicalDevice> Context::enumeratePhysicalDevices(const std::vector<std::string>& requestedExtensions,
                                                                  bool enableRayTracing) const
    {
        ...
        for (const auto device : devices)
        {
            physicalDevices.emplace_back(
                PhysicalDevice(device, surface_, requestedExtensions, printEnumerations_, enableRayTracing));
        }
        return physicalDevices;
    }

    PhysicalDevice Context::choosePhysicalDevice(std::vector<PhysicalDevice>&& devices,
