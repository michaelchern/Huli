...
namespace Huli::Vulkan
{
    ...
    CommandQueueManager::CommandQueueManager(const Context& context,
                                             VkDevice device,
                                             uint32_t count,
          ...
          device_(device)
    {
        ...
        context.setVkObjectname(commandPool_, VK_OBJECT_TYPE_COMMAND_POOL, "Command pool: " + name);
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            context.setVkObjectname(
                cmdBuffer, VK_OBJECT_TYPE_COMMAND_BUFFER, "Command buffer: " + name + " " + std::to_string(i));
    ...
    void CommandQueueManager::waitUntilSubmitIsComplete()
    {
        ...
        const auto result = vkWaitForFences(device_, 1, &fences_[fenceCurrentIndex_], true, UINT32_MAX);
