...
namespace Huli::Vulkan
{
    ...
    {
        ASSERT(actualBufferIfStaging_, "Actual Buffer must not be null in case of staging buffer");
        ...
        VK_CHECK(vmaCreateBuffer(allocator_, &createInfo, &allocCreateInfo_, &buffer_, &allocation_, nullptr));
    ...
    {
        VK_CHECK(vmaCreateBuffer(allocator_, &createInfo, &allocInfo, &buffer_, &allocation_, nullptr));
        vmaGetAllocationInfo(allocator_, allocation_, &allocationInfo_);
    ...
    {
        // we could control the offset & size as well if needed
                                          ...
                                          uint64_t dstOffset) const
    {
        ...
        ASSERT(actualBufferIfStaging_ != nullptr, "actualBufferIfStaging_ can't be null in case of staging");
        vkCmdCopyBuffer(commandBuffer, vkBuffer(), actualBufferIfStaging_->vkBuffer(), 1, &region);
    }
