...
namespace
{
    ...
    {
    public:
        explicit CustomIncluder(const std::string& shaderDir)
...
namespace Huli::Vulkan
{
                                                ...
                                                const char* entryPoint)
    {
        ...
        if (shaderStage == EShLangRayGen || shaderStage == EShLangAnyHit || shaderStage == EShLangClosestHit ||
        ...
        const EShMessages messages =
            static_cast<EShMessages>(EShMsgDefault | EShMsgSpvRules | EShMsgVulkanRules | EShMsgDebugInfo);
        CustomIncluder includer(shaderDir);

        std::string preprocessedGLSL;
        if (!tshadertemp.preprocess(resources, 460, ENoProfile, false, false, messages, &preprocessedGLSL, includer))
        {
    ...
    void ShaderModule::createShader(const std::string& filePath, const std::string& entryPoint, const std::string& name)
    {
        ...
        {
            spirv = glslToSpirv(
                fileData, shaderStageFromFileName(filePath.c_str()), file.parent_path().string(), entryPoint.c_str());
        }
#endif
                                    ...
                                    const std::string& name)
    {
        ...
        context_->setVkObjectname(vkShaderModule_, VK_OBJECT_TYPE_SHADER_MODULE, "Shader Module: " + name);
    }
