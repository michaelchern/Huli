...
namespace Huli::Vulkan
{
    ...
    Texture::Texture(const Context& context,
                     VkImageType type,
                     VkFormat format,
          ...
          debugName_(name)
    {
        ...
        VK_CHECK(vmaCreateImage(
            vmaAllocator_, &imageInfo, &allocCreateInfo, &image_, &vmaAllocation_, nullptr));
        ...
        const VkImageViewType imageViewType = imageTypeToImageViewType(type, flags, multiview_);

        viewType_ = imageViewType;

        imageView_ = createImageView(context, imageViewType, format_, mipLevels_, layerCount, name);
    }
    ...
    void Texture::uploadAndGenMips(VkCommandBuffer cmdBuffer,
                                   const Buffer* stagingBuffer,
                                   void* data)
    {
        uploadOnly(cmdBuffer, stagingBuffer, data);
        context_.beginDebugUtilsLabel(
            cmdBuffer, "Transition to Shader_Read_Only & Generate mips", {1.0f, 0.0f, 0.0f, 1.0f});
    ...
    void Texture::uploadOnly(VkCommandBuffer cmdBuffer,
                             const Buffer* stagingBuffer,
                             void* data,
                             uint32_t layer)
    {
    ...
    void Texture::addReleaseBarrier(VkCommandBuffer cmdBuffer,
    ...
    void Texture::addAcquireBarrier(VkCommandBuffer cmdBuffer,
                                    uint32_t srcQueueFamilyIndex,
                                    uint32_t dstQueueFamilyIndex)
    {
    ...
    void Texture::transitionImageLayout(VkCommandBuffer cmdBuffer, VkImageLayout newLayout)
    {
        ...
        vkCmdPipelineBarrier(
            cmdBuffer, sourceStage, destinationStage, 0, 0, nullptr, 0, nullptr, 1, &barrier);
    ...
    bool Texture::isStencil() const
    {
        return (format_ == VK_FORMAT_S8_UINT || format_ == VK_FORMAT_D16_UNORM_S8_UINT ||
                format_ == VK_FORMAT_D24_UNORM_S8_UINT || format_ == VK_FORMAT_D32_SFLOAT_S8_UINT);
    }
    ...
    void Texture::generateMips(VkCommandBuffer cmdBuffer)
    {
        ...
        vkGetPhysicalDeviceFormatProperties(
            context_.physicalDevice().vkPhysicalDevice(), format_, &formatProperties);
    ...
    std::vector<std::shared_ptr<VkImageView>> Texture::generateViewForEachMips()
    {
        ...
        for (int i = 0; i < mipLevels_; ++i)
        {
            ...
            VK_CHECK(
                vkCreateImageView(context_.device(), &imageViewInfo, nullptr, output.back().get()));
    ...
    VkImageView Texture::createImageView(const Context& context,
