...
namespace Huli::Vulkan
{
        ...
        : physicalDevice_{device}
    {
        ...
        // Enumerate extensions
        {
            ...
            VK_CHECK(vkEnumerateDeviceExtensionProperties(physicalDevice_, nullptr, &propertyCount, nullptr));
            std::vector<VkExtensionProperties> properties(propertyCount);
            VK_CHECK(vkEnumerateDeviceExtensionProperties(physicalDevice_, nullptr, &propertyCount, properties.data()));

            std::transform(properties.begin(),
                           properties.end(),
                           std::back_inserter(extensions_),
                           [](const VkExtensionProperties& property) { return std::string(property.extensionName); });
        ...
        {
            std::cerr << properties_.properties.deviceName << " " << properties_.properties.vendorID << " ("
                      << properties_.properties.deviceID << ") - ";
            const auto apiVersion = properties_.properties.apiVersion;
            std::cerr << "Vulkan " << VK_API_VERSION_MAJOR(apiVersion) << "." << VK_API_VERSION_MINOR(apiVersion) << "."
                      << VK_API_VERSION_PATCH(apiVersion) << "." << VK_API_VERSION_VARIANT(apiVersion) << ")"
                      << std::endl;
    ...
    void PhysicalDevice::reserveQueues(VkQueueFlags requestedQueueTypes, VkSurfaceKHR surface)
    {
             ...
             ++queueFamilyIndex)
        {
            if (!presentationFamilyIndex_.has_value() && surface != VK_NULL_HANDLE)
            {
                ...
                vkGetPhysicalDeviceSurfaceSupportKHR(physicalDevice_, queueFamilyIndex, surface, &supportsPresent);
            ...
            if (!graphicsFamilyIndex_.has_value() &&
                (requestedQueueTypes & queueFamilyProperties_[queueFamilyIndex].queueFlags) & VK_QUEUE_GRAPHICS_BIT)
            {
            ...
            if (!computeFamilyIndex_.has_value() &&
                (requestedQueueTypes & queueFamilyProperties_[queueFamilyIndex].queueFlags) & VK_QUEUE_COMPUTE_BIT)
            {
            ...
            if (!transferFamilyIndex_.has_value() &&
                (requestedQueueTypes & queueFamilyProperties_[queueFamilyIndex].queueFlags) & VK_QUEUE_TRANSFER_BIT)
            {
    ...
    [[nodiscard]] std::vector<std::pair<uint32_t, uint32_t>> PhysicalDevice::queueFamilyIndexAndCount() const
    {
        ...
        std::vector<std::pair<uint32_t, uint32_t>> returnValues(familyIndices.begin(), familyIndices.end());
        return returnValues;
    }
