...
#include <vulkan/CommandQueueManager.hpp>
...
int main(int argc, char** argv)
{
    ...
    // Create Context
    Huli::Vulkan::Context::enableDefaultFeatures();
    Huli::Vulkan::Context context((void*)glfwGetWin32Window(window_),
    ...
    const auto fragShader = context.createShaderModule(fragShaderPath.string(), VK_SHADER_STAGE_FRAGMENT_BIT);

    // Create Framebuffers
    std::vector<std::shared_ptr<Huli::Vulkan::Framebuffer>> swapchain_framebuffers(context.swapchain()->numberImages());

    // Create Render Pass
    std::shared_ptr<Huli::Vulkan::RenderPass> renderPass = context.createRenderPass({context.swapchain()->texture(0)},
    ...
    auto pipeline = context.createGraphicsPipeline(
        Huli::Vulkan::Pipeline::GraphicsPipelineDescriptor{
    ...
    while (!glfwWindowShouldClose(window_))
    {
        ...
        if (swapchain_framebuffers[swapchainImageIndex] == nullptr)
        {
            swapchain_framebuffers[swapchainImageIndex] =
                context.createFramebuffer(renderPass->vkRenderPass(), {texture}, nullptr, nullptr);
        }
