...
Huli 是一个 C++20 / Vulkan 学习仓库。当前仓库只保留最小 CMake 骨架、AI 工作流和学习记录结构；具体教材、实验代码和构建目标以后按需加入。
...
- 不要回滚用户已有改动。
- 不要假设尚未加入仓库的教材、源码目录、构建目标或第三方依赖已经存在。
...
- `CMakeLists.txt`：当前最小构建骨架；尚未定义项目和可构建目标。
...
C++ / CMake / shader / 示例改动按需运行最小相关验证。当前仓库根 `CMakeLists.txt` 仍是学习骨架；如果构建入口尚未完善，先明确“未配置可构建目标”，不要把构建失败说成 Vulkan 代码失败。
...
- 不要修改文件。
- 检查范围由实际中文源动态发现，包括缺失目标、孤立英文文件、重复 marker 和明显未翻译的中文正文。
- 如果不同步，提示用户运行 `=sa`。
...
- 长任务恢复、证据、TODO 状态和失败探索使用 `docs/tasks/zh-CN/study-template.md` 的轻量结构；不要默认创建并行状态目录。
